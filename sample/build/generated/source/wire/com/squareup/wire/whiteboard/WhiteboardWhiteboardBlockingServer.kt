// Code generated by Wire protocol buffer compiler, do not edit.
// Source: com.squareup.wire.whiteboard.Whiteboard in com/squareup/wire/whiteboard/whiteboard.proto
package com.squareup.wire.whiteboard

import com.squareup.wire.MessageSink
import com.squareup.wire.MessageSource
import com.squareup.wire.Service
import com.squareup.wire.WireRpc
import kotlin.Unit

public interface WhiteboardWhiteboardBlockingServer : Service {
  @WireRpc(
    path = "/com.squareup.wire.whiteboard.Whiteboard/Whiteboard",
    requestAdapter = "com.squareup.wire.whiteboard.WhiteboardCommand#ADAPTER",
    responseAdapter = "com.squareup.wire.whiteboard.WhiteboardUpdate#ADAPTER",
    sourceFile = "com/squareup/wire/whiteboard/whiteboard.proto",
  )
  public fun Whiteboard(request: MessageSource<WhiteboardCommand>,
      response: MessageSink<WhiteboardUpdate>): Unit
}
